...
echo "Setting attribute iam_admin_resource_server=true..."

"${KCADM}" update "clients/${CLIENT_UUID}" \
  -r "${REALM}" \
  -s 'attributes.iam_admin_resource_server=true'

echo "Done. Client '${CLIENT_ID}' is now marked as iam_admin_resource_server."
