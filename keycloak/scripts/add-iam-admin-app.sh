...
echo "    not create them. On a fresh realm nothing is needed: the application"
echo "    creates the artifacts when a function is attached to an organization."
echo ""
echo "    If the realm already holds organizations, run a reconciliation so this"
echo "    client gets its scopes, policies, permissions and optional client scope"
echo "    bindings for them. Log in to the IAM Admin application as a superuser"
echo "    (create-admin-user.sh) and press 'Reconcile All' on the Services tab."
echo "    The tab flags a client whose artifacts are missing, so it also shows"
echo "    whether this step is pending."
