...
#
# Nothing else on the client is changed. In particular iam_admin_managed is left
# as it is, so a managed OIDC client keeps that role and ends up holding both.
#
...
Mark an existing Keycloak client as a resource server administered by the IAM
admin application by setting the iam_admin_resource_server=true client attribute.
The client must already exist; use add-resource-server.sh to create one.
...
echo "==> Setting iam_admin_resource_server=true..."
...
echo ""
echo "==> Client '${CLIENT_ID}' is now marked as iam_admin_resource_server."
