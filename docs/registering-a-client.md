...
[Keycloak Setup §2.10](keycloak-setup.md#managed-clients-and-reconciliation). The rights
model behind the `{org}:{function}:{right}` scopes is in the
...
- For the GUI route: you are logged in to the admin application as a **superuser**. Every
  client operation is superuser-only; a non-superuser does not see the **Services** tab at
  all, and the API answers `403`.
...
has just been registered, but not a working one — see
...
The client takes the OIDC client shape — confidential, `client-jwt`, standard flow,
Authorization Services — and carries both marker attributes.
...
A service account is what gives a client Keycloak Admin API access — the IAM admin
...
- **It cannot be deleted from the GUI.** The delete button is disabled, and
  `DELETE /api/clients/{uuid}` answers `409` for such a client — deleting it would take away
