...
# This attribute is read by the resource-aud plugin at token issuance time
# to validate that the requested function matches the target resource server.
#
...
Options:
  ...
  --functions <functions>  Comma-separated list of functions (e.g. demo,walletreg)
...
[ -z "${FUNCTIONS}" ] && { read -r -p "Functions (comma-separated, e.g. demo,walletreg): " FUNCTIONS; }
...
echo "==> Setting client_functions='${FUNCTIONS}'..."
CURRENT=$(api_get "/${REALM}/clients/${CLIENT_UUID}")
UPDATED=$(CURRENT_JSON="${CURRENT}" _FUNCTIONS="${FUNCTIONS}" python3 -c "
import os, json
client = json.loads(os.environ['CURRENT_JSON'])
if not client.get('attributes'):
    client['attributes'] = {}
client['attributes']['client_functions'] = os.environ['_FUNCTIONS']
