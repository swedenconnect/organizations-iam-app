...
# keycloak/scripts/add-oidc-client.sh, the single implementation of this script, with the
...
# container.
#
# Against any other Keycloak, call keycloak/scripts/add-oidc-client.sh directly.
...
TARGET="${REPO_ROOT}/keycloak/scripts/add-oidc-client.sh"
CACERT="${REPO_ROOT}/compose/config/common/tls.crt"

if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
  cat <<EOF
Register an OIDC relying-party client with private_key_jwt authentication,
Authorization Services and the IAM protocol mappers.
