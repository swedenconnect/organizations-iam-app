...
# keycloak/scripts/add-iam-admin-app.sh, the single implementation of this script, with the
...
# container.
#
# Against any other Keycloak, call keycloak/scripts/add-iam-admin-app.sh directly.
...
TARGET="${REPO_ROOT}/keycloak/scripts/add-iam-admin-app.sh"
CACERT="${REPO_ROOT}/compose/config/common/tls.crt"

if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
  cat <<USAGE
Register the IAM admin application in Keycloak as an OIDC client, a resource server, and a
client handling all functions. Use this rather than add-oidc-client.sh, which gives it only
the OIDC client role.

Runs against the compose Keycloak at ${KC_URL}.

Usage: $0 --realm <realm> --username <username> --password <password> --client-id <clientId> [OPTIONS]
...
Options:
  ...
  --client-id <clientId>    Client ID, the IAM admin application base URL
  --name <name>             Display name. Default: IAM Admin
  --redirect-uri <uri>      Redirect URI, repeatable. Default: /login/oauth2/code/*
  --root-url <url>          Root URL. Default: the client ID
  --jwks-url <url>          JWKS URL. Default: <client id>/jwks
