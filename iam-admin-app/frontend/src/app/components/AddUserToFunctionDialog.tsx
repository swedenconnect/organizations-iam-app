...
import { Organization, User, UserOrganizationRole, FunctionType } from '@/types';
...
export function AddUserToFunctionDialog({
  open,
  onClose,
  organization,
  func,
  ...
  const availableUsers = users.filter((u) => {
    if (!organization || !func) return true;
    if (u.id === currentUserId) return false;
    return !userRoles.some(
      (r) => r.organizationId === organization.id && r.functionId === func.id && r.userId === u.id
    );
  });
  ...
  const handleSelectAndAdd = () => {
    if (organization && func && selectedUserId) {
      onAddUserToFunction(organization.id, func.id, selectedUserId, selectedRole);
      onClose();
    }
  };
  ...
  if (!organization || !func) return null;
      ...
      <DialogContent className="sm:max-w-[560px]">
        <DialogHeader>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {getOrgName(organization)} — {(language === 'sv' ? func.nameSv : func.nameEn) || (language === 'sv' ? func.nameEn : func.nameSv) || func.name}
          </p>
        </DialogHeader>
            ...
            {duplicateUser ? (
              ...
              <div className="space-y-4">
                ...
                <DialogFooter>
                  ...
                  <Button
                    onClick={() => { onAddUserToFunction(organization!.id, func!.id, duplicateUser.id, selectedRole); onClose(); }}
                    className="bg-primary hover:bg-primary/90"
                  >
                    {t('org.addUser')}
                  </Button>
              ...
              <div className="space-y-4">
                ...
                <DialogFooter>
                  ...
                  <Button
                    onClick={() => {
                      onAddUserToFunction(organization!.id, func!.id, externalDuplicateUserId, selectedRole);
                      onClose();
                    }}
                    className="bg-primary hover:bg-primary/90"
                  >
              ...
              <>
                ...
                <div className="space-y-1">
                  ...
                  <Input type="tel" value={newPhone} onChange={(e) => setNewPhone(e.target.value)} placeholder="" />
                </div>
                ...
                {fieldErrors.general && <p className="text-xs text-red-500">{fieldErrors.general}</p>}
